...
"""
ex usage: fetch_count(item_type="AP", start_date=[2014,6,1], end_date=[2014,6,2])
...
def fetch_count(item_type, start_date, end_date=datetime.now()):
    ...
    res = requests.get(URL % { "item_type":item_type
                             ...
                             , "end_date": end_date})
    total = json.loads(res.content).get("total")
    #print total
    return total
...
def report():
     ...
     article_count = fetch_count(item_type="AP", start_date=[2014,6,1], end_date=[2014,6,2])
     print "This is the article count ", article_count
     return None
